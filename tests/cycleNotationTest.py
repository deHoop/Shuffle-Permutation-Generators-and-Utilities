...
class TestCycleNotation(unittest.TestCase):
    ...
    def test_prime(self):
        ...
        numpy.testing.assert_array_equal(cycles, [])

    def test_perfect_square_of_primes(self):
        n = 9 
        gens = permutations.permutations(n)
        cycles = cycleNotation.cycleNotation(gens, n)
        numpy.testing.assert_array_equal(cycles, [[[2,4], [3,7], [6,8]]])

    def test_composite(self):
        n = 12
        gens = permutations.permutations(n)
        cycles = cycleNotation.cycleNotation(gens, n)
        numpy.testing.assert_array_equal(cycles, [[[2, 4, 5, 3], [8, 10, 11, 9]], 
            [[4, 7], [5, 8], [6, 9]], 
            [[2, 7, 4, 8, 10, 11, 6, 9, 5, 3]], 
            [[2, 4, 10, 6, 5], [3, 7, 8, 11, 9]], 
            [[2, 7, 5], [3, 4, 10, 9], [6, 8, 11]], 
            [[2, 3, 5, 4], [8, 9, 11, 10]], 
            [[3, 7, 9, 5], [4, 8, 10, 6]], 
